...
struct TCP_segment seg1, seg2, response; // send, send, receive
...
int receive_pkt(int server_fd, char *str, struct TCP_segment seg)
{
	...
	if (recv(server_fd, &response, sizeof(response), 0) < 0) { 
		if (errno == EAGAIN || errno == EWOULDBLOCK) {
			...
			if (send(server_fd, &seg, sizeof(seg), 0) < 0) {
				perror("retransmit send failed");
				return -1;
			}
			printf("\t(#)Retransmitted a packet : %s => SEQ=%d : ACK=%d\n", str, seg.sequence_num, seg.acknowledgment);
...
void perform_file_transfer(char *name, int server_fd) {
	...
	if (simulate_packet_loss()) {
		...
		seg1.checksum = 1; // indicate bad packet
	}
	else 
		seg1.checksum = 0;
	...
	int count = 0;
	int delay = 0; // used to wait for two packets
	while(1)
	{
		// receive response and retransmit if happening time_out situation
		int re;
		if(count == 0)
		{
			count++;
			re = receive_pkt(server_fd, "PSH", seg1);
		}
		else
			re = receive_pkt(server_fd, "PSH", seg2);
			
		if(re < 0) {
			perror("receive failed, expecting PSH-ACK");
		    close(server_fd);
		    exit(EXIT_FAILURE);
		}
		...
		if(delay == 2) // wait until waiting for two packets
		{
			delay = 0; // reset
			...
			send(server_fd, &seg2, sizeof(seg2), 0); // send the ack after receiving the second packet
...
void perform_dns_query(char *name, int server_fd){
	...
	// receive response and retransmit if happening time_out situation
	int re = receive_pkt(server_fd, "PSH", seg1);
	if(re < 0) {
		perror("receive failed, expecting PSH-ACK");
		close(server_fd);
		exit(EXIT_FAILURE);
...
int main(int argc, char**argv) 
{
  ...
  //  }
    ...
    // receive response and retransmit if happening time_out situation
	int re = receive_pkt(sock, "SYN", seg1);
	if(re < 0) {
		perror("receive failed, expecting SYN-ACK");
		close(sock);
		exit(EXIT_FAILURE);
	}
